...
class PlayerApp(App):
    ...
    async def _initialize_game(self):
        ...
        try:
            ...
            self.game_client = AsyncGameClient(
                base_url=self.server_url, transport="websocket", character_id=self.character_id
            )
            ...
            self.task_manager = TaskManager(
                ...
                task_complete_callback=lambda was_cancelled, via_stop_tool: asyncio.create_task(
                    self._on_task_complete(was_cancelled, via_stop_tool)
                ),
            ...
            self.chat_widget.add_message(
                "system",
                "Debug mode enabled - showing chat agent messages. Press Ctrl+D to toggle.",
            )
    ...
    def _progress_callback(self, text: str, status: str):
        ...
        if status == "start":
            ...
            status == "complete" or status == "failed" or status == "cancelled" or status == "error"
        ):
    ...
    async def _update_state(self):
        ...
        try:
            ...
            if new_sector != self.current_sector:
                ...
                self.movement_history.add_movement(self.current_sector, new_sector, port_code)
                self.current_sector = new_sector
    ...
    async def _on_task_complete(
        self, was_cancelled: bool = False, via_stop_tool: bool = False
    ) -> None:
    ...
    def action_toggle_debug(self) -> None:
        ...
        if self.debug_mode:
            ...
            self.debug_widget.styles.display = "block"
            self.chat_widget.add_message(
                "system", "Debug mode enabled - showing chat agent messages"
            )
        else:
