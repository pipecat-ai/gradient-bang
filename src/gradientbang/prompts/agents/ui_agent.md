...
- `show_player_ship_tab: "ships"|"cargo"|"modules"|"strategy"` switches the player ship panel tab. Only use `"strategy"` when the user explicitly asks to **see / show / view** their ship's combat strategies (e.g. "show me my ship's strategies", "show the ship strategy"). NOT a general strategy catch-all — questions like "what's a good strategy?" or "should I attack?" are voice agent territory and need no UI change.
...
Ship strategies tab example — narrow scope, user explicitly asks to view their ship strategies:
User: "Show me my ship's strategies." / "Show the ship strategy."
→ `control_ui(show_player_ship_tab="strategy")`
Do NOT fire this for general strategy talk ("what's the best strategy here?", "should I retreat?") — those are voice agent territory.
