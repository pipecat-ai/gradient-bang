...
def apply_combat_event(
    ...
    if event.event_type == "ship.destroyed":
        # Escape pods are gameplay-indestructible; spurious ship.destroyed events
        # for them appear when a pod re-enters combat through a server-side gap.
        # Filter here so headline counts reflect real hull losses.
        ship_type = clean_str(event.payload.get("ship_type"))
        ship_name = clean_str(event.payload.get("ship_name"))
        if ship_type == "escape_pod" or ship_name == "Escape Pod":
            return
