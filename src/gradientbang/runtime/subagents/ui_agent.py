...
from gradientbang.runtime.inference_gate import PreLLMInferenceGate
from gradientbang.runtime.tool_schema import CORPORATION_INFO, MY_STATUS
from gradientbang.utils.llm_factory import create_llm_service, get_ui_agent_llm_config
...
class UIAgentContext(FrameProcessor):
    ...
    def __init__(self, config, rtvi, game_client) -> None:
        ...
        self._character_id = game_client.character_id
        ...
        self._status_timeout_secs = settings.UI_AGENT_STATUS_TIMEOUT_SECS
        self._ports_list_timeout_secs = settings.UI_AGENT_PORTS_LIST_TIMEOUT_SECS
        self._ships_list_timeout_secs = settings.UI_AGENT_SHIPS_LIST_TIMEOUT_SECS
        self._course_plot_timeout_secs = settings.UI_AGENT_COURSE_PLOT_TIMEOUT_SECS
        self._ports_list_stale_secs = settings.UI_AGENT_PORTS_LIST_STALE_SECS
        self._intent_request_delay_secs = settings.UI_AGENT_INTENT_REQUEST_DELAY_SECS
    ...
    def _read_ships_cache_ttl() -> int:
        return max(0, int(settings.UI_AGENT_SHIPS_CACHE_TTL_SECS))
    ...
    async def handle_corporation_info(self, params: FunctionCallParams) -> None:
        ...
        async def _fetch():
            try:
                ...
                elif arguments.get("corp_id"):
                    result = await self._game_client._request(
                        "corporation.info",
                        {"character_id": self._character_id, "corp_id": arguments["corp_id"]},
                    )
                else:
                    result = await self._game_client._request(
...
class UIAgent:
    """Parallel-branch UI agent: bundles the three FrameProcessors that form
    the UI control branch and registers their tool handlers on a dedicated LLM.

    bot.py uses this as:
        ui_agent = UIAgent.create(rtvi=..., game_client=...)
        ParallelPipeline([voice_branch], ui_agent.branch)
    ...
    def __init__(
        ...
        *,
        ...
        llm: Any,
        collector: UIAgentResponseCollector,
    ) -> None:
        self._context = context
        self._llm = llm
        self._collector = collector
        # Cache the branch list so the same processor instances flow into
        # both ParallelPipeline and RTVI ignored_sources.
        self._branch: list[FrameProcessor] = [context, llm, collector]

    @classmethod
    def create(cls, *, rtvi: Any, game_client: Any) -> "UIAgent":
        config = get_ui_agent_llm_config()
        context = UIAgentContext(config=config, rtvi=rtvi, game_client=game_client)
        llm = create_llm_service(config)
        collector = UIAgentResponseCollector(context=context)
        agent = cls(context=context, llm=llm, collector=collector)
        agent._register_functions()
        return agent

    def _register_functions(self) -> None:
        self._llm.register_function("control_ui", self._context.handle_control_ui)
        self._llm.register_function("queue_ui_intent", self._context.handle_queue_ui_intent)
        self._llm.register_function("corporation_info", self._context.handle_corporation_info)
        self._llm.register_function("my_status", self._context.handle_my_status)

    @property
    def branch(self) -> list[FrameProcessor]:
        return self._branch
