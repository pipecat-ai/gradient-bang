...
def calculate_price_sell_to_player(
    commodity: str, stock: int, max_capacity: int
) -> int:
...
def calculate_price_buy_from_player(
    commodity: str, stock: int, max_capacity: int
) -> int:
...
def get_port_stock(port_data: Dict) -> Dict[str, int]:
    ...
    for commodity, key in commodity_keys.items():
        stock[commodity] = port_data.get("stock", {}).get(key, 0)
...
def validate_buy_transaction(
    ...
    if player_credits < total_cost:
        raise TradingError(
            f"Insufficient credits. Need {total_cost}, have {player_credits}"
        )
...
def validate_sell_transaction(
    ...
    if player_cargo.get(commodity, 0) < quantity:
        raise TradingError(
            f"Not enough {commodity} to sell. Have {player_cargo.get(commodity, 0)}"
        )

    available_capacity = port_max_capacity - port_stock
    if available_capacity < quantity:
        raise TradingError(
            f"Port can only buy {available_capacity} units of {commodity}"
        )
