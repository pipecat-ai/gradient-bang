...
BEGIN
  ...
  INSERT INTO quest_definitions (id, code, name, description, assign_on_creation, enabled)
  VALUES (
    gen_random_uuid(),
    'tutorial',
    'Getting Started',
    'Learn the basics of trading, navigation, and survival in the galaxy.',
    true,
    true
  )
  RETURNING id INTO v_quest_id;

  -- Step 1: Travel to any adjacent sector
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value)
  VALUES (v_step_id, v_quest_id, 1,
    'Travel to any adjacent sector',
    'Use the warp drive to move to a neighboring sector.',
    'count', '{movement.complete}', 1);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('movement.complete', v_step_id);

  -- Step 2: Locate the Megaport
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value, payload_filter)
  VALUES (v_step_id, v_quest_id, 2,
    'Locate the Megaport',
    'Find a sector that contains a Megaport.',
    'count_filtered', '{movement.complete}', 1,
    '{"has_megaport": true}'::jsonb);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('movement.complete', v_step_id);

  -- Step 3: Refuel your ship
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value)
  VALUES (v_step_id, v_quest_id, 3,
    'Refuel your ship',
    'Purchase warp fuel to recharge your drives.',
    'count', '{warp.purchase}', 1);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('warp.purchase', v_step_id);

  -- Step 4: Purchase a commodity
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value)
  VALUES (v_step_id, v_quest_id, 4,
    'Purchase a commodity',
    'Buy goods from a port to begin trading.',
    'count', '{trade.executed}', 1);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('trade.executed', v_step_id);

  -- Step 5: Earn 2000 credits trading
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value, aggregate_field)
  VALUES (v_step_id, v_quest_id, 5,
    'Earn 2000 credits trading',
    'Generate at least 2000 credits in trading profit.',
    'aggregate', '{trade.executed}', 2000,
    'profit');
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('trade.executed', v_step_id);

  -- Step 6: Deposit 2000 credits into the megabank
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value, payload_filter)
  VALUES (v_step_id, v_quest_id, 6,
    'Deposit 2000 credits into the megabank',
    'Transfer at least 2000 credits to the Megaport bank.',
    'count_filtered', '{bank.transaction}', 1,
    '{"amount_gte": 2000}'::jsonb);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('bank.transaction', v_step_id);

  -- Step 7: Purchase a kestrel
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value, payload_filter)
  VALUES (v_step_id, v_quest_id, 7,
    'Purchase a kestrel',
    'Buy a Kestrel Courier from a shipyard.',
    'count_filtered', '{ship.traded_in}', 1,
    '{"ship_type": "kestrel_courier"}'::jsonb);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('ship.traded_in', v_step_id);

  -- Step 8: Create or join a corporation
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value)
  VALUES (v_step_id, v_quest_id, 8,
    'Create or join a corporation',
    'Form a new corporation or join an existing one.',
    'count', '{corporation.created,corporation.member_joined}', 1);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('corporation.created', v_step_id);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('corporation.member_joined', v_step_id);

  -- Step 9: Run a task on a corp ship
  v_step_id := gen_random_uuid();
  INSERT INTO quest_step_definitions (id, quest_id, step_index, name, description, eval_type, event_types, target_value, payload_filter)
  VALUES (v_step_id, v_quest_id, 9,
    'Run a task on a corp ship',
    'Purchase or select a corporation ship and execute a task on it.',
    'count_filtered', '{task.start}', 1,
    '{"ship_owner": "corporation"}'::jsonb);
  INSERT INTO quest_event_subscriptions (event_type, step_id) VALUES ('task.start', v_step_id);

  RAISE NOTICE 'Tutorial quest seeded with id: %', v_quest_id;
END;
$$;
