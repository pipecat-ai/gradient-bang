...
function parseManifest(raw: unknown): Record<string, number> {
  ...
  if (Array.isArray(raw)) {
    for (const entry of raw) {
      ...
      iterator.push([
        (entry as Record<string, unknown>)["commodity"],
        (entry as Record<string, unknown>)["units"],
      ]);
    }
  } else if (typeof raw === "object") {
    for (const [commodity, units] of Object.entries(
  ...
  for (const [commodityRaw, unitsRaw] of iterator) {
    if (
      typeof commodityRaw !== "string" ||
      !VALID_COMMODITIES.has(commodityRaw)
    ) {
      throw new DumpCargoError(`Invalid commodity: ${commodityRaw}`, 400);
    }
    if (
      typeof unitsRaw !== "number" ||
      !Number.isInteger(unitsRaw) ||
      unitsRaw <= 0
    ) {
...
async function handleDumpCargo(
  ...
  if (ship.in_hyperspace) {
    throw new DumpCargoError(
      "Character is in hyperspace, cannot dump cargo",
      400,
    );
  }
