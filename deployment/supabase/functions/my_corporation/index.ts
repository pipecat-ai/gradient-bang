...
async function loadMyCorporation(params: {
  ...
  const corporation = await loadCorporationSafe(supabase, character.corporation_id);
  const members = await fetchCorporationMembers(supabase, character.corporation_id);
  const ships = await fetchCorporationShipSummaries(supabase, character.corporation_id);
