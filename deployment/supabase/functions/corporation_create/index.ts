...
async function handleCreate(params: {
  ...
  if (shipCredits < CORPORATION_CREATION_COST) {
    throw new CorporationCreateError(`Insufficient credits (need ${CORPORATION_CREATION_COST.toLocaleString()})`, 400);
  }

  const inviteCode = generateInviteCode();
  const inserted = await insertCorporation({ supabase, name: normalizedName, founderId: characterId, inviteCode });
  const joinedAt = inserted.founded ?? new Date().toISOString();

  await upsertCorporationMembership(supabase, inserted.corp_id, characterId, joinedAt);
  await updateShipCredits(supabase, ship.ship_id, shipCredits - CORPORATION_CREATION_COST);
...
async function insertCorporation(params: {
  ...
  founderId: string;
  inviteCode: string;
}): Promise<{ corp_id: string; name: string; founded: string; invite_code: string }> {
  ...
  if (error) {
    if (error.code === '23505') {
      throw new CorporationCreateError(`Corporation name '${name}' already taken`, 400);
    }
  ...
  return data as { corp_id: string; name: string; founded: string; invite_code: string };
}
...
function ensureActorMatches(actorId: string | null, characterId: string): void {
  if (actorId && actorId !== characterId) {
    throw new CorporationCreateError('actor_character_id must match character_id for corporation.create', 400);
  }
}
