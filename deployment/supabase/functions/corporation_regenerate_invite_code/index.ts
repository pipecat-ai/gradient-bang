...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    await enforceRateLimit(supabase, characterId, 'corporation_regenerate_invite_code');
  } catch (err) {
...
async function handleRegenerate(params: {
  ...
  if (!isMember) {
    throw new CorporationInviteError('Not authorized for this corporation', 403);
  }
  ...
  const source = buildEventSource('corporation_regenerate_invite_code', requestId);
  const payload = {
    source,
    corp_id: corpId,
...
function ensureActorMatches(actorId: string | null, characterId: string): void {
  if (actorId && actorId !== characterId) {
    throw new CorporationInviteError('actor_character_id must match character_id for corporation.regenerate_invite_code', 400);
  }
}
