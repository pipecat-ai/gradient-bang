...
export async function fetchActiveTaskIdsByShip(
  ...
  const { data: startCharacterRows, error: startCharacterError } = await supabase
    .from('events')
    .select('ship_id, character_id, task_id, event_type, inserted_at')
    .eq('event_type', 'task.start')
    .eq('direction', 'event_out')
  ...
  const combinedStarts = [...(startShipRows ?? []), ...(startCharacterRows ?? [])];
  ...
  for (const row of combinedStarts) {
    if (!row || (typeof row.ship_id !== 'string' && typeof row.character_id !== 'string')) {
      continue;
    }
    const shipKey = typeof row.ship_id === 'string' && row.ship_id
      ? row.ship_id
