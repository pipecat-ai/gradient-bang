...
// Mirrors loadCorpDeliveryReasons in _shared/events.ts but via direct PG, so
// pgRecordEvent's pgmq fan-out can run without a SupabaseClient. Returns the
// union of corp-owned ship pseudo-chars (reason=corp_ship) and active corp
// members (reason=corp_member); members win for any id present in both sets.
async function pgLoadCorpDeliveryReasons(
  pg: QueryClient,
  corpId: string,
): Promise<Map<string, string>> {
  const map = new Map<string, string>();

  const ships = await pg.queryObject<{ ship_id: string }>(
    `SELECT ship_id::text AS ship_id FROM ship_instances
     WHERE owner_type = 'corporation' AND owner_corporation_id = $1::uuid`,
    [corpId],
  );
  for (const row of ships.rows) {
    if (typeof row.ship_id === "string") map.set(row.ship_id, "corp_ship");
  }

  const members = await pg.queryObject<{ character_id: string }>(
    `SELECT character_id::text AS character_id FROM corporation_members
     WHERE corp_id = $1::uuid AND left_at IS NULL`,
    [corpId],
  );
...
interface PgPublishEventToPgmqOptions {
  pg: QueryClient;
  eventType: string;
  scope: string;
  direction: string;
...
// PG-direct mirror of publishEventToPgmq in _shared/events.ts. Same recipient
// expansion + per-recipient pgmq.send + broadcast notify_broadcast, but issued
// on the same `pg` connection that just wrote the events row — no supabase RPC
// roundtrip. Each individual send is wrapped in try/catch so one failed
// recipient doesn't block the rest, and the events INSERT (in a separate prior
// statement) is unaffected by any pgmq error.
async function pgPublishEventToPgmq(
  opts: PgPublishEventToPgmqOptions,
): Promise<void> {
  ...
  if (opts.corpId) {
    try {
      const corpReasonMap = await pgLoadCorpDeliveryReasons(
        opts.pg,
        opts.corpId,
      );
      ...
      console.error("pgRecordEvent.pgmq_publish.corp_expand_failed", {
  ...
  if (opts.broadcast) {
    ...
    try {
      await opts.pg.queryObject(
        "SELECT public.notify_broadcast($1::jsonb)",
        [JSON.stringify(broadcastMsg)],
      );
    } catch (err) {
      console.error("pgRecordEvent.pgmq_publish.broadcast_send_failed", {
        eventType: opts.eventType,
        err: err instanceof Error ? err.message : err,
      });
    }
  ...
  for (const [recipient, reason] of reasonByRecipient) {
    ...
    try {
      await opts.pg.queryObject(
        "SELECT public.pgmq_publish($1::text, $2::jsonb)",
        [queueName, JSON.stringify(msg)],
      );
    } catch (err) {
      console.error("pgRecordEvent.pgmq_publish.send_failed", {
        eventType: opts.eventType,
        queueName,
        err: err instanceof Error ? err.message : err,
      });
...
export async function pgRecordEvent(
  ...
  // failure here cannot affect polling subscribers. The outer try/catch is
  // belt-and-braces; individual sends already swallow their own errors.
  try {
    await pgPublishEventToPgmq({
      pg,
      eventType,
      scope,
      direction,
    ...
    console.error("pgRecordEvent.pgmq_publish_failed", {
      eventType,
      err: err instanceof Error ? err.message : err,
    });
  }
