...
Deno.serve(traced("combat_action", async (req, trace) => {
  ...
  try {
    ...
    sHandle.end();
    trace.setOutput({ request_id: requestId, combat_id: combatId, action: actionRaw });
    return result;
  } catch (err) {
...
async function handleCombatAction(params: {
  ...
  if (!encounter || encounter.ended) {
    const err = new Error("Combat encounter not found") as Error & {
      status?: number;
    };
    err.status = 404;
    throw err;
  ...
  if (!participant || participant.combatant_type !== "character") {
    const err = new Error("Character not part of this combat") as Error & {
      status?: number;
    };
    err.status = 403;
    throw err;
  ...
  console.log("combat_action.round_check", {
    ...
    participant_details: Object.entries(encounter.participants).map(
      ([pid, p]) => ({
        pid,
...
function normalizeAction(value: string): CombatantAction {
  ...
  const err = new Error(`Unknown combat action: ${value}`) as Error & {
    status?: number;
  };
  err.status = 400;
  throw err;
...
async function buildActionState(params: {
  ...
  if (action === "attack") {
    if (participant.fighters <= 0) {
      const err = new Error("No fighters available for attack") as Error & {
        status?: number;
      };
      err.status = 400;
      throw err;
    ...
    if (!success) {
      const err = new Error(
        "Toll payment failed - no toll garrison found or insufficient credits",
      ) as Error & { status?: number };
      err.status = 400;
      throw err;
    }
...
async function processTollPayment(
  ...
  try {
    ...
    const amount =
      typeof entry.toll_amount === "number" ? entry.toll_amount : 0;
    ...
    const currentBalance =
      typeof entry.toll_balance === "number" ? entry.toll_balance : 0;
    ...
    const sources = context.garrison_sources as
    ...
    if (ownerId) {
      ...
      if (garrisonUpdateError) {
        ...
        console.error(
          "processTollPayment.garrison_update",
          garrisonUpdateError,
        );
      }
    }
