...
// ============================================================================
// Group 5: fetchActiveTaskIdsByShip reads from column (via ships.list event)
// ============================================================================

Deno.test({
  name: "ship_task_lock — fetchActiveTaskIdsByShip reads from column",
  sanitizeOps: false,
  sanitizeResources: false,
  async fn(t) {
    let corpShipId: string;
    ...
    await t.step("seed corp + start task", async () => {
      await resetDatabase([P1]);
      await apiOk("join", { character_id: p1Id });
      await setShipCredits(p1ShipId, 50_000);
      const seeded = await seedCorpWithShip(p1Id, "Active Task Corp");
      corpShipId = seeded.corpShipId;
      await apiOk("task_lifecycle", {
        character_id: corpShipId,
        actor_character_id: p1Id,
    ...
    let cursor: number;
    await t.step("capture cursor before list_user_ships", async () => {
      cursor = await getEventCursor(p1Id);
    });

    await t.step(
      "list_user_ships emits ships.list with current_task_id from column",
      async () => {
        ...
        const corpShipRow = ships.find((s) => s.ship_id === corpShipId);
        assertExists(corpShipRow);
        assertEquals(corpShipRow.current_task_id, taskId);
      },
    );
  },
});
