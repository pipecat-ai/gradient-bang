...
def load_universe_data(filepath="world-data/universe_structure.json"):
...
def analyze_universe(filepath="world-data/universe_structure.json"):
    """Run all connectivity tests on the universe."""
    print(f"\n{'='*60}")
    print(f"UNIVERSE CONNECTIVITY ANALYSIS")
    print(f"{'='*60}\n")
    
    # Load data
    print(f"Loading universe data from: {filepath}")
    universe_data = load_universe_data(filepath)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        filepath = "world-data/universe_structure.json"
    
    success = analyze_universe(filepath)
