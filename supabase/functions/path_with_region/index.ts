...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (err instanceof PathWithRegionError || err instanceof PathNotFoundError) {
      ...
      return errorResponse(err.message, err instanceof PathWithRegionError ? err.status : 400);
    }
...
async function handlePathWithRegion(
  ...
  if (!Number.isInteger(regionHops) || regionHops < 0) {
    throw new PathWithRegionError('region_hops must be a non-negative integer', 400);
  }
  ...
  if (!Number.isInteger(maxSectors) || maxSectors <= 0 || maxSectors > MAX_ALLOWED_SECTORS) {
    throw new PathWithRegionError(`max_sectors must be between 1 and ${MAX_ALLOWED_SECTORS}`, 400);
  }
