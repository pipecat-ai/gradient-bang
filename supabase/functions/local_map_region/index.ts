...
async function handleLocalMapRegion(
  ...
  if (!knowledge.sectors_visited[String(centerSector)]) {
    throw new LocalMapRegionError(`Center sector ${centerSector} must be a visited sector`, 400);
  }
  ...
  if (!Number.isInteger(maxHops) || maxHops < 0 || maxHops > 10) {
    throw new LocalMapRegionError('max_hops must be an integer between 0 and 10', 400);
  }
  ...
  if (!Number.isInteger(maxSectors) || maxSectors <= 0) {
    throw new LocalMapRegionError('max_sectors must be a positive integer', 400);
  }
