...
- `SUBAGENT_BUS_DATABASE_URL` (Postgres DSN) and `SUBAGENT_BUS_CHANNEL` are **both required** when `SUBAGENT_BUS_TRANSPORT=pgmq`. Channel has no default — `PgmqBus` broadcasts on publish to every peer queue sharing the channel prefix, so a default would cross-talk bus traffic between concurrent bots sharing a database. Set a per-deployment value (e.g. `gb_prod`, `gb_dev_jon`); external BYOA agents must use the same value as the bot they're talking to
