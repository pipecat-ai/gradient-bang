...
- `TaskAgent._inject_steering` now injects steer text as two user messages: a `<event name="task.steered">` directive header ("User has steered your task. Override and prioritize your current plan with the instruction below.") followed by the raw steer text as a normal user message. Same event-xml shape as `task.progress` / `task.cancelled` the agent already reads. Orchestrator side drops its now-redundant `"Steering instruction: "` text prefix.
