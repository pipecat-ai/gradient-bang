...
export const tunnelFragmentShader = `
  ...
  void main() {
    ...
    // Sample pre-baked noise texture (angle → U, time scroll → V)
    float timeOffset = uTime * speed * 0.3;
    float noiseU = angle / TAU;
    float noiseV = (timeOffset + noiseAnimationSpeed * uTime) * 0.2;
    float val = texture2D(noiseTex, vec2(noiseU, noiseV)).r;
