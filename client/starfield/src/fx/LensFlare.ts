...
/**
 * Interface for lens flare effect options
 */
export interface LensFlareEffectOptions {
  blendFunction?: BlendFunction
  enabled?: boolean
  glareSize?: number
  lensPosition?: [number, number] | Vector2
  iResolution?: [number, number] | Vector2
  starPoints?: number
  flareSize?: number
  flareShape?: number
  anamorphic?: boolean
...
/**
 * Fragment shader for lens flare effect
 */
const fragmentShader = /* glsl */ `
  ...
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    if(aditionalStreaks) {
      vec3 o = vec3(0.9, 0.2, 0.1), p = vec3(0.3, 0.1, 0.9);
      for(float n = 0.0; n < 10.0; n++) {
        f += drC(g, pow(rnd(n * 2e3) * 2.8, 0.1) + 1.41, 0.0, o + n, p + n, rnd(n * 20.0) * 3.0 + 0.2 - 0.5, lensPosition);
      }
    }
    
    if(secondaryGhosts) {
      vec3 n = vec3(0);
      n += rHx(g, -lensPosition * 0.25, ghostScale * 1.4, vec3(0.25, 0.35, 0));
      n += rHx(g, lensPosition * 0.25, ghostScale * 0.5, vec3(1, 0.5, 0.5));
      n += rHx(g, lensPosition * 0.1, ghostScale * 1.6, vec3(1));
      n += rHx(g, lensPosition * 1.8, ghostScale * 2.0, vec3(0, 0.5, 0.75));
      n += rHx(g, lensPosition * 1.25, ghostScale * 0.8, vec3(1, 1, 0.5));
    ...
    if(starBurst) {
      ...
      n += mix(n, pow(n * 2.0, vec4(2)) * 0.5, o);
    ...
    outputColor = enabled ? vec4(mix(f, vec3(0), opacity) + inputColor.xyz, inputColor.w) : vec4(inputColor);
  }
`

/**
 * Implementation of the lens flare effect
 * Creates realistic lens flare effects with customizable parameters
 */
export class LensFlareEffect extends Effect {
  ...
  /**
   * 3D position of the light source for the lens flare
   */
  sourcePosition: Vector3
  ...
  /**
   * Maximum distance for occlusion detection
   */
  occlusionDistance: number
   ...
   */
  private opacityTarget: number = 0.0
  ...
  /**
   * Temporary vector for screen position
   */
  private screenPosition: Vector3

  /**
   * Creates a new lens flare effect instance
  ...
  constructor({
    ...
    opacity = 0.0,
    starBurst = true,
    sourcePosition = new Vector3(0, 0, -100),
    enableOcclusion = true,
    occlusionDistance = 1000,
  }: LensFlareEffectOptions = {}) {
    // Convert array to Vector2 if needed
    const lensPos = Array.isArray(lensPosition)
      ? new Vector2(lensPosition[0], lensPosition[1])
      : lensPosition
    const resolution = Array.isArray(iResolution)
      ? new Vector2(iResolution[0], iResolution[1])
      : iResolution
    ...
    const uniforms = new Map<string, Uniform>([
      ...
      ["lensPosition", new Uniform(lensPos)],
      ["iResolution", new Uniform(resolution)],
    ...
    this.occlusionDistance = occlusionDistance
  ...
  /**
   * Updates the lens flare position and occlusion
   * Call this from your render loop with the current camera and scene
   * @param camera - The camera to use for projection
   * @param scene - The scene to raycast against (optional, for occlusion)
   */
  updatePosition(camera: Camera, scene?: Scene): void {
    // Project source position to screen space
    this.screenPosition.copy(this.sourcePosition)
    this.screenPosition.project(camera)

    // Check if source is behind camera
    const isBehindCamera = this.screenPosition.z > 1

    // Update lens position uniform
    const lensPos = this.uniforms.get("lensPosition")
    if (lensPos) {
      lensPos.value.x = this.screenPosition.x
      lensPos.value.y = this.screenPosition.y
    }

    // Occlusion detection
    if (this.enableOcclusion && scene && !isBehindCamera) {
      ...
      const sourceDistance = (camera as any).position.distanceTo(
        this.sourcePosition
      )

      // Check if anything is blocking the light source
      for (const intersect of intersects) {
        // Only check mesh objects
        if (intersect.object instanceof THREE.Mesh) {
          const mesh = intersect.object

          // Skip if intersection is beyond the occlusion distance
          if (intersect.distance > this.occlusionDistance) {
            continue
          }

          // If intersection is closer than the source, it's occluded
          if (intersect.distance < sourceDistance) {
      ...
      // Set target opacity based on occlusion
      this.opacityTarget = isOccluded ? 1.0 : 0.0
    } else if (isBehindCamera) {
      // Source is behind camera, hide flare
      this.opacityTarget = 1.0
    } else {
      // Occlusion disabled, always visible
      this.opacityTarget = 0.0
    }

    // Smooth opacity transition
    const currentOpacity = this.uniforms.get("opacity")?.value || 0
    const easedOpacity = THREE.MathUtils.lerp(
      currentOpacity,
      this.opacityTarget,
      0.1
    )
    this.opacity = easedOpacity
  }
  ...
  /**
   * Gets the current glare size
   */
  get glareSize(): number {
    return this.uniforms.get("glareSize")!.value
  }
  ...
  set glareSize(value: number) {
    this.uniforms.get("glareSize")!.value = value
  }
