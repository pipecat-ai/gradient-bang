...
import { ArrowsLeftRightIcon, HandCoinsIcon, VaultIcon } from "@phosphor-icons/react"
      ...
      >
        <CardContent className="flex flex-col h-full justify-between items-center">
          ...
          <div className="flex flex-row gap-3 w-full justify-center items-center">
            {/* Bank: left (order-0) for withdraw, right (order-2) for deposit */}
            <ToastValue
              className={cn(
                "py-2 flex-1",
                meta?.direction === "deposit" ?
                  "order-2 numerical-badge-flashing-increment"
                : "order-0"
              )}
            >
              <VaultIcon weight="duotone" className="size-6" />
              <CurrencyCounter value={creditsInBankBalance} className="text-lg" />
              <span
                className={cn(
                  "text-xs font-medium animate-in fade-in-0 duration-1000",
                  meta?.direction === "deposit" ? "text-success" : "text-warning"
                )}
              >
                {meta?.direction === "deposit" ? "+" : "-"}
                {meta?.amount ?? 0}
            ...
            {/* Arrow: always middle */}
            <ArrowsLeftRightIcon size={24} className="order-1 text-subtle" />

            {/* Hand: left (order-0) for deposit, right (order-2) for withdraw */}
            <ToastValue
              className={cn(
                "py-2 flex-1",
            ...
            >
              ...
              <CurrencyCounter value={creditsOnHandBalance} className="text-lg" />
              <span
                className={cn(
                  "text-xs font-medium animate-in fade-in-0 duration-1000",
                  meta?.direction === "withdraw" ? "text-success" : "text-warning"
                )}
              >
                {meta?.direction === "withdraw" ? "+" : "-"}
                {meta?.amount ?? 0}
