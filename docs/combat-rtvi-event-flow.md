...
| Voice spoken notification | LLM append + `run_llm` decision per `combat_pov`. Direct combat speaks on active combat updates. Observed corp ship/garrison combat speaks on initiation and terminal `combat.round_resolved`; continuing rounds append silently. Sector-only observers remain non-speaking. | `_should_run_llm` in `event_relay.py` |
...
| --- | --- | --- | --- | --- |
| Combat starts via `combat_initiate` | `combat.round_waiting` | One sector-scoped event. Recipients are direct combat participants, current sector observers, and corp members of participant/garrison corps. Recipient reasons come from `computeEventRecipients`: `direct`, `sector_snapshot`, `garrison_owner`, `garrison_corp_member`, `corp_member`. | Every recipient gets `combat.round_waiting`. Participants and observers receive the same non-personalized payload. | Participants enter/update combat UI. Non-participants receive the RTVI frame, but `GameContext` ignores it when the current player is not in `participants[]`. Voice LLM speaks round 1 for stake observers (`observed_via_corp_ship` / `observed_via_garrison`) at normal priority; sector-only observers are silent. |
...
| Round resolves and combat continues | `combat.round_resolved`, then `combat.round_waiting` | Each event is sector-scoped. Recipients are direct remaining participants, current sector observers, and corp members of participant/garrison corps. | Recipients get `combat.round_resolved`, then `combat.round_waiting`. | Participants update combat state/results. Non-participant observers receive RTVI frames, but the app ignores combat state if the current player is not in `participants[]`. Voice LLM appends continuing updates silently for stake observers. |
| Round resolves and combat ends normally | `combat.round_resolved`, `combat.ended`, then `sector.update` | `combat.round_resolved` is sector-scoped to participants/observers/corp members. `combat.ended` is personalized and written once per participant recipient via direct character events; corp members of participant corps and garrison owners/corps also receive a separate observer-safe `combat.ended` with `observed=true` and no personalized `ship` block. `sector.update` is sector-scoped to current sector visibility recipients. | Resolution recipients get `combat.round_resolved`. Each participant gets their own personalized `combat.ended`. Corp/garrison stake observers get observer-safe `combat.ended`. Sector occupants/garrison observers get `sector.update`. | Participant clients show the final round and clear combat on personalized `combat.ended`. Corp/garrison stake observers can clear combat UI indicators from observer-safe `combat.ended`; voice still speaks the outcome from terminal `combat.round_resolved`, not from `combat.ended`. |
...
| Ship defeated | `combat.round_resolved`, optional `salvage.created`, `ship.destroyed`, `combat.ended`, `sector.update` | `salvage.created` is sector-scoped to sector visibility recipients. `ship.destroyed` is sector-scoped to sector observers plus corp members for corp ships. `combat.ended` is personalized for participants and observer-safe for stake observers. | Recipients get the relevant salvage/destruction events plus combat terminal events. | App tracks destroyed ships from `ship.destroyed`; sector contents are refreshed by `sector.update`. Stake observers can clear combat indicators from observer-safe `combat.ended`. Voice speaks `ship.destroyed` for the owning player or for matching corp-ship losses, including non-local corp ships. |
| Garrison defeated | `combat.round_resolved`, `garrison.destroyed`, `combat.ended`, `sector.update` | `garrison.destroyed` is sector-scoped to sector visibility recipients plus owner/corp recipients computed from garrison owner data. `combat.ended` is personalized for participants and observer-safe for stake observers. | Owner/corp/sector recipients get `garrison.destroyed`. Participants get personalized terminal combat events if combat ends; stake observers get observer-safe `combat.ended`. | App removes the garrison from map sector state and adds an activity log entry. Stake observers can clear combat indicators from observer-safe `combat.ended`. Voice speaks `garrison.destroyed` only for the owning character; corp members may receive silent context. |
...
| Corp member observing corp ship combat | Yes through `corp_member` when participant payload exposes corp id, including observer-safe terminal `combat.ended`. | Yes. | No unless their own active player is a participant; client can still track/clear stake combat indicators from observer-safe `combat.ended`. | Round-1 waiting appends with `combat_pov="observed_via_corp_ship"` and triggers inference at normal `event` priority. Continuing `round_waiting` / `round_resolved` append silently. Terminal `round_resolved` and matching `ship.destroyed` trigger normal-priority inference. Observer-safe `combat.ended` is RTVI/UI only. |
| Garrison owner / garrison corp member | Yes through garrison visibility and corp membership, including observer-safe terminal `combat.ended`. | Yes. | Does not enter personal combat UI unless also a participant; client can still track/clear stake combat indicators from observer-safe `combat.ended`. | Round-1 waiting appends with `combat_pov="observed_via_garrison"` and triggers inference at normal `event` priority. Continuing updates append silently. Terminal `round_resolved` triggers normal-priority inference. `garrison.destroyed` triggers voice inference for the direct owner only. Observer-safe `combat.ended` is RTVI/UI only. |
