...
- 2025-10-16: Move RPC now returns minimal acknowledgments (`{"success": True}`); route status rendering through `movement.complete` and related events instead of RPC payloads, and update tool/client expectations accordingly.
...
- 2025-10-16: `combat.initiate` now mirrors the minimal-response pattern; capture the initiator in round-one `combat.round_waiting` payloads so client summaries can attribute the opener when we refactor AsyncGameClient.
- 2025-10-16: AsyncGameClient methods now return minimal RPC acknowledgments and expose `get_event_queue`; queue UI CLI refactors (Phase 5.2) to consume `status.snapshot`, `course.plot`, and related events instead of RPC payloads.
- 2025-10-16: Replaced all server-side `rpc_failure` returns with `HTTPException` + `error` event emissions so RPC failures arrive as `ok: false` frames while keeping event-driven notifications consistent; updated AsyncGameClient to synthesize events only as a safety net.
...
- 2025-10-16 (Ticket 5.1): Refactored `AsyncGameClient` to return minimal RPC acknowledgments, added per-event queues with a `get_event_queue` helper, introduced explicit event delivery gating (pause/resume buffering) so clients can delay processing until ready, registered ExperimentalTaskAgent handlers for all emitted events (logging only for now), updated unit tests, and refreshed the design doc snippet. Tests: `uv run pytest tests/test_utils.py tests/test_event_summaries.py -q` (pass), `uv run pytest tests/test_experimental_pipecat_task_agent.py -q` (pass). Follow-up: migrate TUI/NPC utilities to consume `status.snapshot`, `course.plot`, and `movement.complete` events directly before running the full integration suite.
