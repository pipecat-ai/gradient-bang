...
The server should deliver the client the most recent map knowledge for each character, in response to the /api/my_map request. Beyond that minimal functionality, all world knowledge "thinking" should be done on the client side.
...
3. Calling /api/my_map from run_npc.py does not work. Fix this. Update the run_npc.py code to be able to use the new AsyncGameClient find port and port pairs tools.
