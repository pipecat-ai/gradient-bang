...
# Add tests directory to path so we can import config
_tests_path = REPO_ROOT / 'tests'
if str(_tests_path) not in sys.path:
    sys.path.insert(0, str(_tests_path))

# Import SUPABASE_WORKDIR from centralized config
try:
    from config import SUPABASE_WORKDIR
except ImportError:
    # Fallback if config not available
    SUPABASE_WORKDIR = Path.cwd() / 'deployment' / 'supabase'
    if not SUPABASE_WORKDIR.exists():
        SUPABASE_WORKDIR = Path.cwd() / 'supabase'
        if not SUPABASE_WORKDIR.exists():
...
def pytest_configure(config):
    ...
    try:
        ...
        if supabase_dir:
            ...
            # Update both local and config module
            SUPABASE_WORKDIR = resolved_path
            try:
    ...
    print(f"\n[edge conftest] Supabase directory: {SUPABASE_WORKDIR}")
...
def _cli_args(*args: str) -> list[str]:
    ...
    # Add --workdir before subcommand: supabase --workdir path start
    cmd = [*CLI_COMMAND, '--workdir', str(SUPABASE_WORKDIR)]
    cmd.extend(args)
    return cmd
...
def _ensure_functions_served(names: Tuple[str, ...]) -> None:
    ...
    log_handle.write(f'[pytest] --workdir flag: {SUPABASE_WORKDIR}\n')
    log_handle.write(f'[pytest] Supabase project: {SUPABASE_WORKDIR / "supabase"}\n')
    ...
    print("\n[edge conftest] Starting Supabase functions serve")
    print(f"[edge conftest] Supabase directory: {SUPABASE_WORKDIR}")
    print(f"[edge conftest] Running from: {REPO_ROOT}")
    ...
    if CLI_COMMAND is None:
        raise RuntimeError('Supabase CLI is not available.')
    cmd = [*CLI_COMMAND, '--workdir', str(SUPABASE_WORKDIR), 'functions', 'serve', '--env-file', str(env_file), '--no-verify-jwt']
    
    log_handle.write(f'[pytest] Command: {" ".join(cmd)}\n')
    log_handle.write(f'[pytest] CWD: {REPO_ROOT}\n')
    log_handle.flush()
    print(f"[edge conftest] Command: {' '.join(cmd)}")
...
def supabase_stack():
    ...
    print(f"\n[supabase_stack] Working directory: {SUPABASE_WORKDIR}")
    _load_env()
    if not MANUAL_STACK:
