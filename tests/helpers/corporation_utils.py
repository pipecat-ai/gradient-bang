...
async def reset_corporation_test_state(server_url: str) -> None:
    ...
    USE_SUPABASE = os.environ.get('USE_SUPABASE_TESTS', '').lower() in _TRUTHY
    
    if USE_SUPABASE:
        # Use edge function reset (no client needed, direct SQL reset)
        from helpers.supabase_reset import reset_supabase_state
        reset_supabase_state(character_ids=[])
    else:
        # Legacy FastAPI reset via client
