...
class TestAgentHello:
    ...
    def test_protocol_version_default(self):
        """A response built without specifying protocol_version uses the
        module-level constant. Bumping ``BUS_PROTOCOL_VERSION`` is the
        single place to flag a breaking change."""
        msg = BusAgentHelloResponse(source="task_abc", correlation_id="x")
        assert msg.protocol_version == BUS_PROTOCOL_VERSION


@pytest.mark.unit
class TestSchemaShape:
    """All new dataclasses carry the bus envelope (source + target) and
    one of (correlation_id) or (character_id, task_id) depending on shape.
    Pin down so future field additions don't accidentally drop required
    plumbing."""

    @pytest.mark.parametrize(
        "cls,required_extras",
        [
            (BusGameToolCallRequest, {"correlation_id", "tool_name"}),
            (BusGameToolCallResponse, {"correlation_id"}),
            (BusCombatStrategyRequest, {"correlation_id", "character_id"}),
            (BusCombatStrategyResponse, {"correlation_id"}),
            (BusCorporationQueryRequest, {"correlation_id", "query_type"}),
            (BusCorporationQueryResponse, {"correlation_id"}),
            (BusTaskFinishNotification, {"character_id", "actor_character_id", "task_id", "status"}),
            (BusAgentHelloRequest, {"correlation_id"}),
            (BusAgentHelloResponse, {"correlation_id", "ready", "protocol_version"}),
        ],
    )
    def test_envelope_and_required_fields_present(self, cls, required_extras):
        names = {f.name for f in fields(cls)}
        # Bus envelope from BusDataMessage / BusMessage.
        assert "source" in names
        assert "target" in names
        # Domain-required fields.
        assert required_extras <= names, (
            f"{cls.__name__} missing expected fields: {required_extras - names}"
        )
